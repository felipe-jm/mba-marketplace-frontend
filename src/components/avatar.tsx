    ...
    <div className="relative" onClick={() => setIsOpen(!isOpen)}>
      {data?.seller?.avatar?.url ? (
        <img
          ...
          className="w-12 h-12 rounded-xl object-cover hover:cursor-pointer"
        />
      ) : (
        <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-grayscale-100 hover:cursor-pointer">
          <span className="text-title-md font-poppins font-medium text-grayscale-400">
          ...
          {data?.seller?.avatar?.url ? (
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-grayscale-100">
                <span className="text-title-sm font-poppins font-medium text-grayscale-400">
                  {data?.seller.name?.charAt(0).toUpperCase()}
                </span>
              </div>
              <span className="text-body-sm font-poppins text-grayscale-300">
                {data?.seller.name}
              </span>
            </div>
          )}
