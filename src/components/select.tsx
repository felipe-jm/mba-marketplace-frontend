  ...
  return (
    <div className="relative font-poppins">
      <label className="uppercase text-label-md text-grayscale-200 focus-within:text-orange-base">
        ...
        <div
          className={cn(
            "flex items-center px-[2px] gap-2 py-[14px] border-b transition-colors cursor-pointer",
            {
        ...
        >
          {icon && (
            <Icon
              className={cn("w-6 h-6", { "text-orange-base": isOpen })}
              name={icon}
            />
          )}
          <span
            className={cn(
              "w-full !text-body-md font-normal normal-case",
              selected ? "text-grayscale-400" : "text-grayscale-200"
            )}
          >
          ...
          {selected && (
            <div className="flex items-center rounded-full bg-shape-shape justify-center p-0.5 w-[20px] h-[20px] transition-all hover:filter hover:brightness-90">
              <X
                size={16}
                className="cursor-pointer rounded-full text-grayscale-300"
                onClick={(e) => {
                  e.stopPropagation();
                  setSelected(null);
                  setIsOpen(false);
          ...
          {isOpen ? (
            <ChevronUp className="w-6 h-6 text-grayscale-300" />
          ) : (
            <ChevronDown className="w-6 h-6 text-grayscale-300" />
          )}
        </div>
        {isOpen && (
            ...
            {options.map((option, index) => (
              ...
              >
                ...
                {selected === option.value && (
                  <Icon className="w-5 h-5 ml-auto" name="Check" />
                )}
              </li>
            ))}
          </ul>
      ...
      {error && (
        <div className="flex items-center gap-1 py-2">
          ...
          <span className="text-body-xs text-semantic-danger normal-case">
            {error}
          </span>
        </div>
      )}
