...
export function Dashboard() {
  ...
  const formattedViewsPerDay =
    dayjs(viewsPerDay?.viewsPerDay[0]?.date).format("DD [de] MMMM") +
    " - " +
    dayjs(
      viewsPerDay?.viewsPerDay[viewsPerDay?.viewsPerDay.length - 1]?.date
    ).format("DD [de] MMMM");
        ...
        <div className="grid grid-cols-4 grid-rows-3 gap-4">
          <div>
            <DashboardCardInfo
              ...
              value={String(numberOfProductsSold?.amount)}
              icon="Wallet"
            />
          </div>
          <div className="col-start-1 row-start-2">
            <DashboardCardInfo
              ...
              value={String(numberOfProductsAvailable?.amount)}
              icon="Wallet"
            />
          </div>
          <div className="col-start-1 row-start-3">
            <DashboardCardInfo
              ...
              value={String(numberOfViews?.amount)}
              icon="Wallet"
            />
          </div>
          ...
          <div className=" col-span-3 row-span-3 col-start-2 row-start-1 bg-shape-white p-6 rounded-[20px]">
            <div className="flex flex-col gap-7">
              <div className="flex items-center justify-between">
                <p className="text-title-sm font-sans text-grayscale-500">
                  Visitantes
                </p>
                <div className="flex items-center gap-2">
                  <Icon name="Calendar" size={16} className="text-blue-dark" />
                  <span className="text-body-sm text-grayscale-300 font-poppins">
                    {formattedViewsPerDay}
                  </span>
                </div>
              </div>
