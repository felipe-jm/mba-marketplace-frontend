...
export function VisitorsChart() {
  ...
  function CustomTooltip({
    ...
    active?: boolean;
    payload?: { name: NameType; value: ValueType; payload: { date: string } }[];
  }) {
    if (active && payload && payload.length) {
      return (
        <div className="bg-shape-white drop-shadow-xl rounded-lg flex flex-col gap-2 p-3">
          <p className="text-grayscale-400 text-label-sm font-poppins">
            {dayjs(payload[0]?.payload?.date).format("DD [de] MMMM")}
          </p>
          <div className="flex items-center gap-2 text-grayscale-300 text-body-xs font-poppins">
            <Icon name="Users" className="w-4 h-4" /> {payload[0]?.value}{" "}
            visitantes
          </div>
        </div>
      );
  ...
  return (
    <ResponsiveContainer width="100%" height={266}>
      <LineChart data={data?.viewsPerDay} style={{ fontSize: 12 }}>
        <XAxis
          className="text-grayscale-200 text-label-sm font-poppins"
          dataKey="date"
          tickFormatter={(date) => date && new Date(date).getDate()}
          axisLine={false}
          tickLine={false}
          dy={16}
        />
